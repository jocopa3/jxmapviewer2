...
public class ChessPatternRenderer implements MapRenderer {
     ...
     */
    @Override
    public BufferedImage renderToBitmap(BufferedImage bm, Dimension dimension, int chunkX, int chunkZ, int bX, int bZ, int eX, int eZ, int pX, int pY, int pW, int pL) {

        int x, z, i, j, tX, tY;
        int color;
        System.out.println("chunkX: "+chunkX+", chunkZ: "+chunkZ+", bX: "+bX+", bZ: "+bZ+", eX: "+eX+", eZ: "+eZ+", pX: "+pX+", pY: "+pY+", pW: "+pW+", pL: "+pL);
        System.out.println("w: "+bm.getWidth()+", h: "+bm.getHeight());
